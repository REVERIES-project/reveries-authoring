module.exports = function (app, logger) {
    ...
    app.post('/mlg', function (req, res) {
        var mlg = new MLG()
		let startpageType=req.body.startpageType
		let endpageType=req.body.endpageType
		switch(startpageType){
			case 'youtube':
			break;
			case 'staticmedia':
			startpageType='StaticMedia'
			break
		}
		...
		switch(endpageType){
			...
			break;
			case 'staticmedia':
			endpageType='StaticMedia'
			break
		}

        mlg.label = req.body.label
        mlg.startpage = req.body.startpage
        mlg.startpageType=startpageType
        mlg.endpageType=endpageType
        ...
        if (req.body.itemId && req.body.itemId.length > 0) {
            MLG.findById(req.body.itemId, function (err, toUpdate) {
                if (!toUpdate) {
                    ...
                    toUpdate.startpageType=startpageType
                    toUpdate.endpageType=endpageType
